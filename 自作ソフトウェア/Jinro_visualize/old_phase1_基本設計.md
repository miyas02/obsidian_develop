...
| 機能ID | 大カテゴリ | 中カテゴリ  | 機能                       |
| ---- | ----- | ------ | ------------------------ |
| R01  | 役職    | CO     | COした役職者をRoleDashboardに表示 |
| R02  | 役職    | 能力     | 占い師の占い結果を表示              |
| R03  | 役職    | 能力     | 霊媒師の霊媒結果を表示              |
| R04  | 役職    | 能力     | 騎士の護衛結果を表示               |
| V01  | 投票    | 投票入力   | 投票結果をドラッグアンドドロップで操作できる   |
| V02  | 投票    | タイムライン | 日別の投票履歴の表示               |
| V03  | 投票    | プレイヤー  | プレイヤー別の投票履歴の表示           |
| V04  | 投票    | 分析レポート | 投票分析結果の表示                |
...
classDiagram
	...
	class AbilityResult {
		...
		+midiumResult : MediumResult
		+bgResult : BGResult
		+attackResult : AttackResult
	}
	DayRepository *-- GameDay
	GameDay *-- DiscussionTime
	DiscussionTime *-- VoteHistory
	GameDay *-- nightTime
	nightTime *-- AbilityResult
...
```
# 投票結果画面
## 投票結果画面 class diagram
```mermaid
classDiagram
	class VoteResultContainer {
		+timeline : timelineContainer
		+playerVote : PlayerVoteContaienr
		+voteAnalisisReport : VoteAnalisisReportContainer
	}
	class TimelineContainer {
		
	}
	class PlayerVoteContainer {
	
	}
	class VoteAnalistsReportContainer {
	
	}
	VoteResultContainer *-- TimelineContainer
	VoteResultContainer *-- PlayerVoteContainer
	VoteResultContainer *-- VoteAnalistsReportContainer
