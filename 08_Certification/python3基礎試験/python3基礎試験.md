...
print(text)
```
トリプルクォート`"""`を使うと、リテラル中の開改行が改行文字として反映される。
```python
text = """spam
ham
eggs
...
- **formatメソッド**: 
	`.format(x, y)` を使う場合、`{}` 内に直接変数名を記述することはできず、引数のインデックスやキーワード引数名が必要になります。
```python
name, age = "田中",25
print("名前: {}, 年齢: {}".format(name, age))
print("年齢: {1}, 名前{2}".format(name, age)) # インデックス（番号）で指定
print("名前： {name}, 年齢： {age}".format(name="佐藤", age=30)) # キーワード引数で指定
```
## コーディングスタイル
### コード
...
- 関数
```python
dict.items() # キーとvalueをタプルで取得
enumerate(dict) # キーとインデックスを取得
```
### ラムダ式
...
`write()` : ファイルの書き込み
### Jsonモジュールのメソッド
`dump()` : pythonオブジェクトをJSONにシリアライズ, 戻り値はstr
