...
## Worktree
### 概要
- 同じリポジトリで複数の作業フォルダを持てる機能
- 2つのフォルダに同じリポジトリをcloneするのの簡易版
- フォルダA(worktreeA)でコミットしたら、フォルダBのgitに反映される。ファイル変更はそれぞれのフォルダに閉じられる。コミットしても、それぞれのgitに影響はしない。履歴にでるだけ。
### コマンド
```bash
# 新しいブランチを作成しながら、指定したパスに展開する
git worktree add <パス> -b <新ブランチ>
# どのパスでどのブランチ開かれているか確認
git worktree list
# 指定したワークツリーを削除する
